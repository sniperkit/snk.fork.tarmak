...
func (t *Tarmak) EnvironmentByName(environmentName string) (interfaces.Environment, error) {
	return t.environmentByName(environmentName)
}

func (t *Tarmak) environmentByNameReal(environmentName string) (interfaces.Environment, error) {
...
func (t *Tarmak) Environments() (envs []interfaces.Environment) {
	for _, envConfig := range t.Config().Environments() {
		env, err := t.EnvironmentByName(envConfig.Name)
		if err != nil {
